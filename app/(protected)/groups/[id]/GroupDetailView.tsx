...
interface G {
  ...
  members: {
    student: { StudentID: number; StudentName: string; Email: string };
...
export function GroupDetailView({
  ...
  async function save() {
    try {
      const res = await fetch(`/api/groups/${group.ProjectGroupID}`, {
        method: "PUT",
      ...
      setEditing(false);
      toast.add("Group details saved successfully.", "success");
      setGroup({ ...group, ...form });
    } catch {
      toast.add("Unable to save group details.", "error");
    }
  }
    ...
    try {
      const res = await fetch(`/api/groups/${group.ProjectGroupID}/members`, {
        ...
        body: JSON.stringify({
          ...
          IsGroupLeader: group.members.length === 0,
        }),
      });
      const student = students.find(
        (s) => s.StudentID === Number(selectedStudent),
      );
      if (student) {
        setGroup((prev) => ({
          ...prev,
      ...
      toast.add("Unable to add member.", "error");
    }
  }
  ...
  async function removeMember(sid: number) {
    ...
    try {
      const res = await fetch(
        `/api/groups/${group.ProjectGroupID}/members?studentId=${sid}`,
        { method: "DELETE" },
      );
      if (!res.ok) throw new Error();
      toast.add("Member removed successfully.", "success");
      setGroup({
        ...group,
        members: group.members.filter((m) => m.student.StudentID !== sid),
      });
    } catch {
      toast.add("Unable to remove member.", "error");
    }
  }
  ...
  async function setLeader(sid: number) {
    try {
      ...
      if (!res.ok) throw new Error();
      toast.add("Group leader updated successfully.", "success");
      setGroup({
        ...group,
        members: group.members.map((m) => ({
          ...m,
          IsGroupLeader: m.student.StudentID === sid,
        })),
      });
    } catch {
      toast.add("Unable to update group leader.", "error");
    }
  }
  ...
  async function onUpload(files: File[]) {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      toast.add("File uploaded successfully.", "success");
      window.location.reload();
    } catch {
      toast.add("Unable to upload file.", "error");
    }
  }
    ...
    <div className={styles.wrap}>
      <div className={styles.head}>
        ...
        <p className="text-muted">
          {group.projectType?.ProjectTypeName} · Guide: {group.guide?.StaffName}
        </p>
      </div>
          ...
          {editing ? (
            <div className={styles.formContent}>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, ProjectArea: e.target.value }))
                }
              />
              <textarea
            ...
            <div className={styles.detailsView}>
              <p>
                <strong>Title:</strong> {group.ProjectTitle || "—"}
              </p>
              <p>
                <strong>Area:</strong> {group.ProjectArea || "—"}
              </p>
              <p>
                <strong>Status:</strong>{" "}
                <span
                  className={`${styles.status} ${styles[group.Status.toLowerCase()]}`}
                >
                  {group.Status}
                </span>
              </p>
              {group.ProjectDescription && (
                <p>
                  <strong>Description:</strong> {group.ProjectDescription}
                </p>
              )}
              {canEdit && (
                <Button
                  ...
                  <div className={styles.memberActions}>
                    {m.IsGroupLeader ? (
                      ...
                      canEdit && (
                        <button
                          ...
                          onClick={() => setLeader(m.student.StudentID)}
                        >
                          Set Leader
                        </button>
                      )
                    ...
                    {canEdit && (
                      <button
                        ...
                        onClick={() => removeMember(m.student.StudentID)}
                      >
                        <IconTrash size={14} />
                      </button>
                    )}
                  ...
                  options={[
                    ...
                    ...students
                      .filter(
                        (s) =>
                          !group.members.some(
                            (m) => m.student.StudentID === s.StudentID,
                          ),
                      )
                      .map((s) => ({
